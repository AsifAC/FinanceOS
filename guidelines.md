...
## Supabase Backend Restart - Step 6 Payment Methods Schema - 2026-09-02
...
- The migration has been applied to the live Supabase project.
- Payment methods are user-owned rows, not global/shared records.
- Payment methods are not globally defaulted and are not created during new-user bootstrap.
- Users add their own payment methods because these are private user-specific financial instruments.
- No transactions, expected transactions, savings/debt tables, or storage buckets are created in this step.
...
- `last4` is nullable but must contain exactly four digits when present.
- `color_theme` is nullable and must be either a `#RRGGBB` hex value or a short safe text token when present.
- A partial unique index allows at most one active, non-archived default payment method per user.
- `is_archived` supports hiding old payment methods without deleting them.
...
- `src/types/supabase.ts` includes local types for `payment_methods`.
- `src/services/paymentMethodService.ts` and `src/hooks/usePaymentMethods.ts` provide isolated payment method fetch/create/update/archive/delete/default helpers.
- `public.set_default_payment_method(target_payment_method_id uuid)` provides atomic default switching for the authenticated owner.
- `setDefaultPaymentMethod()` must call the database RPC instead of doing client-side multi-step updates.
- Existing Settings/payment method UI remains on the local FinanceOS store until an explicit integration step.
- Preview/mock payment methods remain separate from Supabase payment methods and must not write to live Supabase data.
...
## Supabase Backend Restart - Step 6B Apply And Verify Payment Methods - 2026-09-02
...
- Public authentication uses a fixed dark theme (`#080A08` / `#050505`), off-white text, restrained borders and the FinanceOS lime accent (`#C6FF00`). It does not inherit the logged-in app's light/dark preference.
- Landing header Login and Sign up are client-side links to `/auth/login` and `/auth/signup`, visible on desktop and mobile. Existing local workspace/preview entry points elsewhere on the landing page remain available. Local setup completion is not treated as Supabase authentication.
- `src/app/components/screens/AuthPage.tsx` uses reusable `AuthLayout`, `AuthForm` and `AuthVisual` components, with scoped styles in `src/styles/auth.css`. AppShell bypasses app navigation/theme chrome for `/auth/` routes. Existing RequireAuth sends unauthenticated users to `/auth/login`; no finance routes were newly protected.
- Desktop uses a 51/49 split with a subtle divider, full viewport minimum height and a centered form capped at 470px. Below 900px, tablet/mobile use a 190–210px visual header and a full-width form within its width cap. Inputs are 52px tall with 16px entry text to avoid iPhone input zoom.
- The original Cash Flow Field uses 48 deterministic off-white vertical bars, with six lime accent overlays. Login has loose waves; signup uses a rising composition. There are no Robinhood assets, logos, text or copied illustrations. The existing FinanceOS dollar-sign mark is reused.
- Motion is driven entirely by staggered CSS transform/opacity keyframes over 8–16 seconds, without timers, frame-based React state, scroll listeners or new dependencies. Mobile/tablet hide half the bars. Accent overlays fade using opacity. `prefers-reduced-motion: reduce` disables continuous animation and transitions while preserving the static visual.
- Login/signup reuse `useAuth`, AuthProvider and authService. Signup adds an optional full name argument, passed as trimmed `options.data.full_name` to existing Auth signup. No profile/preferences writes or database bootstrap changes are made by the UI.
- Login collects email/password; signup adds full name/confirmation. Names and emails are trimmed; passwords are preserved. Required fields, email format and matching passwords are checked locally. No project password-length/complexity policy is recorded locally; Supabase's actual configured policy remains authoritative, with safe weak-password feedback. Verify exact policy in a later authorized Auth validation step before adding more specific client hints.
- Successful Auth sessions redirect to the existing `/dashboard`. A signup user without a session gets a Check your email state and is not treated as authenticated. Copy accounts for Supabase potentially concealing duplicate-account existence. Existing project confirmation-email redirect settings are unchanged and still require live verification.
- AuthPage waits for provider loading to finish before authenticated redirects, shows a compact initial session-loading message, and preserves the form during submission. Forms disable controls and prevent duplicate submissions; pending labels read Logging in… / Creating account….
- Auth errors are mapped to fixed, human-readable messages in the existing service. Network exceptions resolve as sanitized failures so the provider does not stay loading. Missing configuration is presented as temporary account unavailability rather than environment setup instructions on the public page.
- Accessibility includes explicit labels/autocomplete, keyboard focus outlines, a skip-to-form link, field-linked errors, first-invalid-field focus, accessible password reveal controls, live request errors, busy states and decorative aria-hidden animation.
- Password recovery is omitted because no recovery service exists. No artificial Keep me signed in duration or duplicate session infrastructure was added. AuthPanel's earlier foundation remains available but is not mounted on these routes.
- Browser review uses headless Chrome locally with mocked Auth contexts, not live credentials. Layouts were checked at 1440, 1024, 768, 375, 390 and 414px; landing header links at desktop/mobile; no horizontal overflow found. Reduced motion, app light-theme isolation, reveal controls, field focus, mismatched passwords, confirmation without a session and authenticated redirects were exercised. Physical iPhone Safari and real email delivery remain untested.
- Offline Auth service tests are in `tests/authService.test.mjs`; they cover metadata/trim behavior, immediate sessions, safe error mapping, thrown network errors and missing configuration without importing the real client.
- Validation passed: `npm run typecheck`, `npm run build`, `git diff --check` (including separate checks for new files), and all five offline Auth tests. Browser checks also passed for client-side login/signup switching, duplicate-submission prevention and recovery after request failure. Lint unavailable. Existing large-bundle, Node experimental type-stripping and line-ending warnings remain.
- This task changes frontend authentication only. No migrations, RLS, schema, storage, transactions backend files, finance data logic or Preview Mode behavior changed. No MCP/OAuth login or remote changes attempted. Step 8 / expected_transactions remains paused.

## Public Authentication Frontend Refinement - 2026-09-20

- Reused the existing `/auth/login` and `/auth/signup` routes, shared AuthPage/AuthLayout/AuthForm/AuthVisual components, AuthProvider, useAuth and Supabase client. Landing header Login and Sign up links remain accessible on desktop and mobile; successful sessions use the existing `/dashboard` route after provider loading finishes.
- Matched fixed-dark auth tokens to background `#050505`, panel/input `#080A08`, text `#F8FAFC`, secondary text `#A8B3A3`, lime `#C6FF00` and translucent white borders. Desktop retains its split layout; tablet/mobile use a compact branded visual header with full-width form controls.
- Cash Flow Field retains 48 stable bars on desktop and 24 visible bars below 900px, staggered transform/opacity animations and distinct signup placement. Hidden mobile accent animations are now stopped too. Reduced motion disables all continuous movement and preserves a static composition.
- Password visibility buttons now identify their controlled input. A dedicated live status announces login/signup progress. Password-policy rejections mark and describe the password field and remain announced through the request error region.
- Signup continues to delegate actual password policy enforcement to Supabase rather than assuming undocumented project settings. Structured `length`, `characters` and `pwned` rejection reasons are mapped to safe guidance without exposing raw server text. Unknown reasons retain the generic safe error. See [Supabase password security](https://supabase.com/docs/guides/auth/password-security).
- Required/email/matching-password validation, duplicate-submit protection, session-only redirects, confirmation without a session, and safe missing-configuration feedback remain in the shared form. Passwords remain within the active form/request lifecycle. Password recovery is deferred because the existing service has no recovery implementation.
- Added repeatable offline browser review: with the Vite dev server running, run `node tests/authBrowser.mjs`. Requires Node 24 and Chrome; `CHROME_PATH` and `AUTH_REVIEW_URL` can override defaults. The review uses a temporary browser profile and mocked AuthContext, blocks Supabase requests, and writes screenshots outside the repository.
- Validation: typecheck and production build passed; all 7 offline Auth service tests passed. Chrome checks covered login/signup at 375, 390, 414, 768, 1024 and 1440px without horizontal overflow, landing navigation, password reveal, required fields, confirmation, provider-loading redirects, duplicate submissions, request recovery, dark-theme isolation and reduced motion. Desktop login and mobile signup screenshots were visually reviewed. No live authenticated end-to-end test or email delivery test was performed.
- No lint script exists. Existing Vite large-chunk and Node experimental type-stripping warnings remain. No financial data, Preview Mode, backend migrations or live settings were changed. Step 8 remains paused.
